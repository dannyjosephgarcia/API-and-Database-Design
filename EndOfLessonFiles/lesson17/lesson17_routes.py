...
@home_route_blueprint.route("/call_dummy_api/<message>", methods=['GET'])
@inject
def call_dummy_rest_api_example(message, dummy_api_service=Provide[Container.dummy_api_service]):
    response_from_endpoint = dummy_api_service.retrieve_dummy_api_response()
    print(message)
    # Uncomment the line below to generate the query parameters (set in Postman)
    # params = request.args.get('helloWorld')
    response_json = response_from_endpoint.json()
    print(response_json)
    return jsonify(response_from_endpoint)
